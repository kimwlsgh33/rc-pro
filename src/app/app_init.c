...
static error_code_t mc_send_response(const char *res) {
    if (!res) return ERR_INVALID_PARAM;
...
static error_code_t dcps_send_response(const char *packet, int len) {
    if (!packet || len <= 0) return ERR_INVALID_PARAM;
...
static int dcps_get_packet(char *out_buff) {
    ...
    while (uart_rx_available(UART_PORT_2) > 0) {
        if (uart_read_byte(UART_PORT_2, &c) == ERROR_CODE_SUCCESS) {
...
void rm_ev_monitor_state(char st, int position) {
    error_code_t err = safe_format_response(response_buffer.buffer, 
                                          sizeof(response_buffer.buffer),
                                          "MP.ST=%c,%d", 
                                          st, position);
    if (err != ERROR_CODE_SUCCESS) {
        error_record(err);
...
static error_code_t cfg_save(const app_config_t *pc) {
    ...
    ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
        eeprom_write_block(pc, (void *)0, sizeof(app_config_t));
    }
...
void error_handler(error_code_t err) {
    ...
    if ((err & ERR_CAT_CRITICAL) == ERR_CAT_CRITICAL) {
        error_info.severity = ERROR_SEVERITY_CRITICAL;
    } else if ((err & ERR_CAT_SYSTEM) == ERR_CAT_SYSTEM) {
...
static error_code_t process_commands(void) {
    ...
    if (cmd_buffer.buffer && sizeof(cmd_buffer.buffer) > 0) {
        ...
        if (cmd_len > 0) {
            ...
            if (err != ERROR_CODE_SUCCESS) {
                error_record(ERR_COMM_CHECKSUM);
...
static error_code_t send_status_update(void) {
    ...
    error_code_t err = safe_format_response(response_buffer.buffer, 
                                          sizeof(response_buffer.buffer),
                                          "ST=%c,%d,%c,%d,%d,%d,%d,%d,%d,%d",
                                          state.state,
                                          state.rstate,
                                          state.door,
                                          state.monotorPosition,
                                          state.plasticCups,
                                          state.paperCups,
                                          state.purifiedTankLevel,
                                          state.wasteTankLevel,
                                          state.errorCode,
                                          state.errorRoboSeq);
    if (err != ERROR_CODE_SUCCESS) {
        error_record(err);
...
static error_code_t send_rm_state()
{
    ...
    error_code_t err = safe_format_response(response_buffer.buffer, 
                                          ...
                                          "ST=%c,%d,%c,%d,%d,%d,%d,%d,%d,%d", 
                                          rs.state, rs.rstate,
                                          rs.door, rs.monotorPosition, 
                                          rs.plasticCups, rs.paperCups,
                                          rs.purifiedTankLevel, rs.wasteTankLevel, 
                                          rs.errorCode, rs.errorRoboSeq);
    if (err != ERROR_CODE_SUCCESS) {
        error_record(err);
        return err;
    }
